...
import { Download, ExternalLink, ShieldCheck, Clock, QrCode, X, Copy, Check, User, Mail, School, Wallet, Settings } from 'lucide-react';
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400">
                        {currentTab === '#profile' ? 'My Profile' : currentTab === '#settings' ? 'Account Settings' : 'My Digital Wallet'}
                    </h1>
                ...
                <div className="glass-panel px-4 py-2 rounded-xl flex items-center gap-3 w-fit border-blue-500/30">
                    ...
                    <span className="text-sm font-mono text-blue-300">0x71C...97bfE</span>
                </div>
            </div>
                    ...
                    {myCerts.map((cert, idx) => (
                        <div key={idx} className="glass-panel p-6 rounded-3xl flex flex-col h-full hover:-translate-y-1 transition-transform duration-300 group hover:shadow-[0_0_30px_rgba(59,130,246,0.15)] hover:border-blue-500/30">
                            ...
                            <div className="mt-8 pt-4 border-t border-white/10 grid grid-cols-2 gap-3">
                                <button disabled={cert.status !== 'Verified'} className="py-2.5 glass-button rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed">
                                    <Download size={16} /> PDF
                                </button>
                                <button
                                    onClick={() => setShareCert(cert)}
                                    disabled={cert.status !== 'Verified'}
                                    className="py-2.5 bg-blue-600/20 text-blue-400 hover:bg-blue-600/30 border border-blue-500/30 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 glass-input px-4 py-3 rounded-xl bg-black/40">
                                        ...
                                        <span className="text-white/70 font-mono">STU-104523</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 glass-input px-4 py-3 rounded-xl bg-black/40">
                                        ...
                                        <span className="text-white/70">john.doe@university.edu</span>
                                    </div>
                                </div>
                                <div>
                                    <label className="text-xs text-white/40 uppercase tracking-wider mb-1 block">Wallet Address</label>
                                    <div className="flex items-center gap-3 glass-input px-4 py-3 rounded-xl bg-black/40">
                                        <Wallet size={18} className="text-white/40" />
                                        <span className="text-blue-400 font-mono text-sm truncate">0x71C50000000000000097bfE</span>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 min-h-screen overflow-y-auto">
                    ...
                    <div className="bg-[#0a0f1d] p-8 md:p-10 rounded-3xl w-full max-w-2xl relative z-10 animate-in zoom-in-95 duration-200 my-8 shadow-2xl border border-white/10 mx-auto">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-white/10">
                            <button disabled={expandedCert.status !== 'Verified'} className="flex-1 py-3.5 glass-button rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
